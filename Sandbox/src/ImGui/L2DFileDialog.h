...
namespace FileDialog {
	...
	enum class FileDialogType { OpenFile, SaveFile, NoOp };
	...
	inline void ShowFileDialog(bool* open, char* buffer, unsigned int bufferSize,
		...
		static std::string filepathSeperator =
		...
		if (open) {
			...
			if (ImGui::Selectable("..", false, ImGuiSelectableFlags_AllowDoubleClick,
				...
				if (ImGui::IsMouseDoubleClicked(0)) {
					fileDialogCurrentPath =
						std::filesystem::path(fileDialogCurrentPath).parent_path().string();
				}
			}
			for (int i = 0; i < folders.size(); ++i) {
				if (ImGui::Selectable(folders[i].path().stem().string().c_str(),
									  i == fileDialogFolderSelectIndex,
			...
			// set each sort order based on column headings
			if (ImGui::Selectable("File")) {
				...
				fileNameSortOrder =
					(fileNameSortOrder == FileDialogSortOrder::Down ? FileDialogSortOrder::Up
			...
			if (ImGui::Selectable("Size")) {
				...
				typeSortOrder = FileDialogSortOrder::None;
				sizeSortOrder =
					(sizeSortOrder == FileDialogSortOrder::Down ? FileDialogSortOrder::Up
																: FileDialogSortOrder::Down);
			}
			ImGui::NextColumn();
			if (ImGui::Selectable("Type")) {
				...
				sizeSortOrder = FileDialogSortOrder::None;
				typeSortOrder =
					(typeSortOrder == FileDialogSortOrder::Down ? FileDialogSortOrder::Up
																: FileDialogSortOrder::Down);
			}
			ImGui::NextColumn();
			if (ImGui::Selectable("Date")) {
				...
				typeSortOrder = FileDialogSortOrder::None;
				dateSortOrder =
					(dateSortOrder == FileDialogSortOrder::Down ? FileDialogSortOrder::Up
																: FileDialogSortOrder::Down);
			}
			ImGui::NextColumn();
			ImGui::Separator();

			// Sort files based on given sort order
			...
			// display current selected file / folder
			std::string selectedFilePath =
				fileDialogCurrentPath +
				(fileDialogCurrentPath.back() == filepathSeperator.back() ? ""
																		  : filepathSeperator) +
				(fileDialogCurrentFolder.size() > 0 ? fileDialogCurrentFolder
													: fileDialogCurrentFile);
			char* buf = &selectedFilePath[0];
			ImGui::PushItemWidth(724);
			ImGui::InputText("", buf, sizeof(selectedFilePath), ImGuiInputTextFlags_ReadOnly);
			...
			// display delete folder button (w/ style for if usable or not)
			...
			// display new folder popup if needed
			if (ImGui::BeginPopup("NewFolderPopup", ImGuiWindowFlags_Modal)) {
				...
				if (ImGui::Button("Create##1")) {
					if (strlen(newFolderName) <= 0) {
						strncpy(newFolderError, "Folder name can't be empty",
								500 - strlen(newFolderError));
					} else {
				...
				if (ImGui::Button("Cancel##1")) {
					strncpy(newFolderName, "", 500);
					strncpy(newFolderError, "", 500);
					ImGui::CloseCurrentPopup();
				}
			...
			// display delete folder popup if needed
			ImGui::SetNextWindowPos(center, ImGuiCond_Appearing, ImVec2(0.5f, 0.5f));
			...
			// option to cancel out of file dialog
			...
			// saving file popup
			ImGui::SetNextWindowPos(center, ImGuiCond_Appearing, ImVec2(0.5f, 0.5f));
			if (ImGui::BeginPopup("SaveFilePopup", ImGuiWindowFlags_Modal)) {
				...
				if (ImGui::Button("Save##1")) {
					if (strlen(newFileName) <= 0) {
						strncpy(newFileError, "Error: You must name the file!", 500);
					} else {
						strncpy(buffer,
								(fileDialogCurrentPath +
								 (fileDialogCurrentPath.back() == filepathSeperator.back()
				...
				if (ImGui::Button("Cancel##2")) {
					strncpy(newFileName, "", 500);
					strncpy(newFileError, "", 500);
					ImGui::CloseCurrentPopup();
				}

				ImGui::TextColored(ImColor(1.0f, 0.0f, 0.2f, 1.0f), "%s", newFileError);
				ImGui::EndPopup();
			}
